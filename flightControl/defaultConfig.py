...
def getParams():
	...
	defaultParams.desiredPosition=np.array([[-5,5,0],[-5,-5,-4],
		[-5,-5,4] ])  #Agent, amount forward, amount right, absolute altitude, meters
#	defaultParams.desiredPosition = np.array([[[-5,5,2]],[[-5,5,-80]]])
	...
	defaultParams.gains = {'kl':1*np.diag([1,1,1]) , 'ka': 1*np.diag([1,1,1])
		...
		,'kThrottleFF': 0,'kRollFF':1,'cOmega':0.1,'maxEAlt':50,'ki':3,'TRIM_THROT_OFFSET':0}
	defaultParams.config = {'printEvery':50,'ignoreSelfPackets':False,'propagateStates':True , 'geofenceAbort':False
		...
		,'LeaderRotationSource':'Gyro' #Model, Accel, Gyro
