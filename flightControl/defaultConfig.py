...
def getParams():
	...
	defaultParams.desiredPosition = np.array([[[-5, 5, 0], [-5, -5, -4], [-5, -5, 4]],
											 [[-5, 5, 0], [-5, -5, -4], [-5, -5, 4]],
											 [[-5, 5, 0], [-5, -5, -4], [-5, -5, 4]]])
	defaultParams.desiredPosition=np.array( [[[-10,0,-15],[-10,0,15],	[-10,0,30] ],
	...
	defaultParams.gains = {'kl':1*np.diag([1,1,1]) , 'ka': 1.0*np.diag([1,1,1+0*0.3])
		...
		,'maxEHeading':50,'maxEPitch':50,'maxESpeed':500,'a1':2.0,'a2':0.10,'b1':1.0,'b2':0.1,'c1':0.6,'c2':0.05*0 #a for speed, b for heading, c for pitch
		,'maxEAlt':50,'epsD':0.2,'ki':3,'pBarrier':1/255.0
	    ,'hQP': 1e6, 'deltaC':4.0, 'l0q':4.0, 'l1q':4.1, 'ls':2}
